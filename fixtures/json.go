package fixtures

import (
	"database/sql/driver"
	"encoding/json"
)

var (
	_ driver.Valuer = jsonArray{}
	_ driver.Valuer = jsonMap{}
)

type jsonArray []any

func (a jsonArray) Value() (driver.Value, error) {
	return json.Marshal(a)
}

type jsonMap map[string]any

func (m jsonMap) Value() (driver.Value, error) {
	return json.Marshal(m)
}

// Go refuses to convert map[any]any to JSON because JSON only support string keys
// So it's necessary to recursively convert all map[interface]any to map[string]any
func recursiveToJSON(v any) (r any) {
	switch v := v.(type) {
	case []any:
		for i, e := range v {
			v[i] = recursiveToJSON(e)
		}
		r = jsonArray(v)
	case map[any]any:
		newMap := make(map[string]any, len(v))
		for k, e := range v {
			newMap[k.(string)] = recursiveToJSON(e)
		}
		r = jsonMap(newMap)
	default:
		r = v
	}
	return
}
